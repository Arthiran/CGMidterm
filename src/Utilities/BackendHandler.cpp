...
void BackendHandler::GlfwWindowResizedCallback(GLFWwindow* window, int width, int height)
{
	...
	Application::Instance().ActiveScene->Registry().view<ColourCorrectionEffect>().each([=](ColourCorrectionEffect& buf)
	{
		buf.Reshape(width, height);
	});
...
bool BackendHandler::InitGLFW()
{
	...
	window = glfwCreateWindow(800, 800, "CG Midterm - Arthiran Sivarajah - 100660300", nullptr, nullptr);
